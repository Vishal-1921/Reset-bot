...
async def stopnum_command(event):
    ...
    try:
        ...
        try:
            user = await client.get_entity(user_id)
            first_name = user.first_name
            await client.send_message(user_id, f"Hey {first_name} ! Your Number Info Access Has Been Revoked. ⌛🥀 Contact @Hlo_Spidey_Bot To Renew .", parse_mode='markdown')
        except:
            pass
    except ValueError:
...
async def stopfam_command(event):
    ...
    try:
        ...
        try:
            user = await client.get_entity(user_id)
            first_name = user.first_name
            await client.send_message(user_id, f"Hey {first_name} ! Your Family Info Access Has Been Revoked. ⌛🥀 Contact @Hlo_Spidey_Bot To Renew .", parse_mode='markdown')
        except:
            pass
    except ValueError:
        await event.reply("❌ **Invalid User ID!**", parse_mode='markdown')
...
async def stop_command(event):
    ...
    try:
        ...
        try:
            user = await client.get_entity(user_id)
            first_name = user.first_name
            await client.send_message(user_id, f"Hey {first_name} ! Your Access Has Been Revoked. ⌛🥀 Contact @Hlo_Spidey_Bot To Renew .", parse_mode='markdown')
        except:
            pass
    except ValueError:
        await event.reply("❌ **Invalid User ID  !**", parse_mode='markdown')
...
async def private_text_handler(event):
    ...
    if state_type == "waiting_num":
        ...
        days_left = get_user_access_days(user_id, 'family')
        if days_left <= 0 and not is_admin(user_id):
            await event.reply(f"Bro {event.sender.first_name} , You Don't Have Access To This Feature, Get Monthly Access For Family Info in 50rs. Only. Send `Family Access` msg to @Hlo_Spidey_Bot " , parse_mode='markdown')
            del user_state[user_id]
            await delete_user_messages(user_id)
            return
